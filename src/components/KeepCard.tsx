...
const KeepCard = ({ parentContainer, noteTitle, noteContent }: PropsKeepCards) => {
    ...
    function handleOnDragEnd(event:any, info:any){
        //console.log('drag');
        const currentPositionTmp = cardRef.current.getBoundingClientRect();
        ...
        */
        ...
        /*
        let xTmp =  (window.innerWidth/2 -200) - info.point.x  ;
        let Ytmp = (window.innerHeight/2 -200) - info.point.y ; 
        */
    ...
    return (
        ...
        <AnimatePresence> 
        {
            !isBig && (
                <div ref={cardRef}>
                <DraggableCard parentContainer={parentContainer} onClick={handleClick} onDragEnd={handleOnDragEnd}>
                    <motion.div
                    key={noteTitle + 'small'}
                    initial={{ opacity: 0 }}
